...
public class ValidateElectronicsPage extends BaseClass {
	
	@Test
	public void validateElectronicsPage() throws EncryptedDocumentException, IOException {
		...
		dwsHomePage.getElectronicsLinkText().click();
		if(driver.getCurrentUrl().contains(excelUtils.readStringData("testcasedata", 9, 1))) {
			Reporter.log("ElectronicsPage is displayed upon verification of its URL.");
		}else {
			Reporter.log("ElectronicsPage is not displayed upon verification of its URL.");
		}
