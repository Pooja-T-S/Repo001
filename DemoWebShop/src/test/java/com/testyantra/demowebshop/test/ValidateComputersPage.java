...
public class ValidateComputersPage extends BaseClass{
	
	@Test
	public void validateComputersPage() throws EncryptedDocumentException, IOException {
		...
		dwsHomePage.getComputersLinkText().click();
		if(driver.getCurrentUrl().contains(excelUtils.readStringData("testcasedata", 10, 1))) {
			Reporter.log("ComputersPage is displayed upon verification of its URL.");
		}else {
			Reporter.log("ComputersPage is not displayed upon verification of its URL.");
		}
